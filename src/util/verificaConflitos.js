...
function verificaConflitoHorario(startTime, endTime, eleStartTime, eleEndTime) {
    ...
    if (startTime == eleStartTime && endTime == eleEndTime) {
        ...
        throw 'Cobrindo um mesmo horario já exitente'
    }
    //StartTime está entre um horário reservado
    if (startTime > eleStartTime && startTime < eleEndTime) {
        //conflito
        console.log('conflito 1')
        throw 'StartTime está entre um horário reservado'
    }
    //EndTime está entre um horário reservado
    if (endTime > eleStartTime && endTime < eleEndTime) {
        //conflito
        console.log('conflito 2')
        throw 'EndTime está entre um horário reservado'
    }
    //Cobrindo um horario já exitente
    if (startTime <= eleStartTime && endTime >= eleEndTime) {
        //conflito
        console.log('conflito 3')
        throw 'Cobrindo um horario já exitente'
    }

}

module.exports = {
    async verificaConflito(type, startTime, endTime, specificDay, weekDays, startDay, endDay, vehicle = undefined) {
        try {

            var allSpecificDay
            var allWeekDays
            var allIntervalDays
            var allIntervalDaysAndWeekDays

            if (vehicle == undefined) {
                allSpecificDay = await DAOScheduling.getAllSpecificDay();
                allWeekDays = await DAOScheduling.getAllWeekDays();
                allIntervalDays = await DAOScheduling.getAllIntervalDays();
                allIntervalDaysAndWeekDays = await DAOScheduling.getAllIntervalDaysAndWeekDays();
            } else {
                allSpecificDay = await DAOScheduling.getAllSpecificDayByVehicle(vehicle);
                allWeekDays = await DAOScheduling.getAllWeekDaysByVehicle(vehicle);
                allIntervalDays = await DAOScheduling.getAllIntervalDaysByVehicle(vehicle);
                allIntervalDaysAndWeekDays = await DAOScheduling.getAllIntervalDaysAndWeekDaysByVehicle(vehicle);
            }
